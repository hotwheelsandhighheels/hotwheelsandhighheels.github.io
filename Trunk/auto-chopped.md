...
<!-- AI RECEPTION SLIDESHOW -->
<div class="slideshow auto-chopped-slideshow" id="ai-reception-slideshow" data-current="1" data-total="54">
...
<!-- 1. HuffPost -->
  ...
  <div class="slide">
    <img src="/assets/ai/Ao3 Letter.png" alt="An open letter from a member of AO3's Heated Rivalry community" loading="lazy">
...
<div class="slideshow-controls">
  <button class="slide-btn" type="button" onclick="changeAutoChoppedSlide('ai-reception-slideshow', -1)">&#8592;</button>
  <span class="slide-count">
    <span id="ai-reception-slideshow-current">1</span> /
    <span id="ai-reception-slideshow-total">54</span>
  </span>
  <button class="slide-btn" type="button" onclick="changeAutoChoppedSlide('ai-reception-slideshow', 1)">&#8594;</button>
</div>

<script>
function initializeAutoChoppedSlideshows() {
  const slideshows = document.querySelectorAll('.auto-chopped-slideshow');

  slideshows.forEach(slideshow => {
    const slides = Array.from(slideshow.querySelectorAll('.slide'));
    if (!slides.length || !slideshow.id) return;
    ...
    slides.forEach((slide, index) => {
      if (index !== activeIndex) slide.classList.remove('active');
    });
    ...
    slideshow.setAttribute('data-total', total);

    const currentEl = document.getElementById(slideshow.id + '-current');
    const totalEl = document.getElementById(slideshow.id + '-total');
...
function changeAutoChoppedSlide(slideshowId, direction) {
  ...
  let current = parseInt(slideshow.getAttribute('data-current'), 10) || 1;
  const total = slides.length;
  ...
  const currentEl = document.getElementById(slideshowId + '-current');
  const totalEl = document.getElementById(slideshowId + '-total');

  if (currentEl) currentEl.textContent = current;
  if (totalEl) totalEl.textContent = total;
}

initializeAutoChoppedSlideshows();
</script>
